...
class EchoPrepareAgentExecutor(AgentExecutor):
    """
    Agent executor for the Echo Prepare student study and exam preparation agent
    """
    ...
    def __init__(self):
        ...
        self._agent = None
        self._active_tasks = {}
        logger.info("EchoPrepareAgentExecutor initialized")

    async def _get_agent(self, session_id: str, actor_id: str):
        """Get or create the agent instance"""
        if self._agent is None:
            logger.info("Creating Echo Prepare agent...")
            self._agent = create_agent(session_id=session_id, actor_id=actor_id)
            logger.info("Echo Prepare agent created successfully")
        return self._agent
